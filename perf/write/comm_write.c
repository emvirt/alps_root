...
int use_default_buffer(int bytes)
{
	...
	for(i=0; i<MAX_DEFAULT_INT_COUNT; i++) {
		data[i] = i;
	}
	...
	// start timer
	ret = ioctl(dev, GLOBAL_TIMER_START);

	ret = write(dev, data, bytes);
...
int use_large_buffer(int bytes, int utime)
{
	...
	for(i=0; i<data_count; i++) {
		...
		ret = write(dev, cur, (MAX_LARGE_BUF_SIZE + MAX_BUF_SIZE));
	...
	if(remained_data_size > 0) {
		...
		ret = write(dev, cur, (remained_data_size + MAX_BUF_SIZE));
	}

	// reset data transmission mode
