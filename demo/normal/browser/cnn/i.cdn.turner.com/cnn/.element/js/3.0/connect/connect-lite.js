...
function ms_uiGetEmails(jsonResponse) {
    ...
    if (error == false) {
        ...
        for (var i = 0; i < jsonResponse.emails.length; i++) {
            if (jsonResponse.emails[i].isPrimary == 'true' && jsonResponse.emails[i].status != 'confirmed') {
                confirm = false;
            }
        }
        if (confirm == false) {
...
function ms_uiDoGetUser(jsonResponse) {
    ...
    if (error == false && displayname == true) {
        // update ui for successful
        CNN_setCookie('displayname', jsonResponse.user.screenName, 854400, 'http://weather.edition.cnn.com/', document.domain);
...
function ms_doLogout() {
    CNN_removeCookie('the_forum', 'http://weather.edition.cnn.com/', 'cnn.com');
    CNN_removeCookie('authid', 'http://weather.edition.cnn.com/', 'cnn.com');
    CNN_removeCookie('authpass', 'http://weather.edition.cnn.com/', 'cnn.com');
    CNN_removeCookie('displayname', 'http://weather.edition.cnn.com/', 'cnn.com');
    CNN_removeCookie('firstName', 'http://weather.edition.cnn.com/', 'cnn.com');
...
function createLoginIframe() {
    ...
    disqusCallBackPath = disqusCallBackPath.split('http://weather.edition.cnn.com/');
...
function showOverlay(divid) {
    ...
    if (divid == 'profile_signin_overlay') {
        clearInputErrors();
    }
...
if (!ms_isLoggedIn && allCookies['CNN_memberID']) {
    CNN_removeCookie('CNN_memberID', 'http://weather.edition.cnn.com/', document.domain);
}
    ...
    var api = {
        init: function() {
            ...
            (function() {
                ...
                FB_CoreJS.setAttribute('src', '../../../connect.facebook.net/en_US/all.js');
        ...
        setUserDataFB: function(obj, id, reload) {
            ...
            CNN_setCookie('CNN_memberID', Object.toJSON(CNN_FB_user), 854400, 'http://weather.edition.cnn.com/', document.domain);
        ...
        setUserDataCNN: function(obj, fb) {
            CNN_setCookie('displayname', obj.profile.screenName, 854400, 'http://weather.edition.cnn.com/', document.domain);
        ...
        setDisconnectPrefs: function() { //called from disconnect
            ...
            CNN_setCookie('profile.lastNewsID', 'fbfriends', 24 * 30 * 12, 'http://weather.edition.cnn.com/', document.domain);
            CNN_setCookie('CNN_memberID', Object.toJSON(CNN_FB_user), 854400, 'http://weather.edition.cnn.com/', document.domain);
            if (Member.isLoggedIn('cnn')) {
                Member.cnnDisconnectFB();
            } else {
                CNN_setCookie('cnnfb.hasDisconnected', 1, 24 * 30 * 12, 'http://weather.edition.cnn.com/', document.domain);
                window.location.reload();
            }
        },
        setActivityFlag: function() {
            if (Member.hasDisconnected()) {
                CNN_setCookie('cnnfb.hasDisconnected', 0, 24 * 30 * 12, 'http://weather.edition.cnn.com/', document.domain);
                window.location.reload();
            } else {
        ...
        logoutCallback: function(response) {
            ...
            if (sso) {
                ...
                var logoutArgs = 'assertionConsumerUrl%3dhttp_/www.disqus.com/saml/cnn/logout/index.html';
        ...
        executeCookieCopy: function(logout) {
            ...
            var cookieCopy = (logout) ? 'http://audience.ireport.com/services/cnn/cookiecopy.api?drop=true&url=' + escape(CNN.Page.getRefURL({
                'is_LR': '1'
            })) : 'http://audience.ireport.com/services/cnn/cookiecopy.api?cnnid=' + CNNid + '&authid=' + authid + '&authpass=' + authpass + '&displayname=' + displayname + '&firstName=' + firstName + '&url=' + escape(CNN.Page.getRefURL({
                'is_LR': '1'
            })) + '&keepMeLoggedIn=' + Member.getRememberMe();
        ...
        loginCallback: function(response, fb) {
            ...
            if (sso) {
                var disqus = CSIManager.getInstance();
                var loginArgs = 'assertionConsumerUrl%3dhttp_/www.disqus.com/saml/cnn/postback/index.html';
                if (isFB) {
...
//personalization module overlay
    ...
    function CNN_FBpsnlzdOO() {
        ...
        if (Member.isLoggedIn('facebook') && !hasDisconnected) {
            t_html += 'Using Facebook, you can now see what your friends are recommending and sharing on CNN. To learn more, or to disable this feature, go to <a href="http://weather.edition.cnn.com/profile/?setTab=mysettings">settings</a>.';
        } else {
            t_html += 'Using Facebook, you can now see the most recommended CNN stories. Facebook user? If so, <a href="javascript:CNN_handleOverlay(\'profile_signin_overlay\');">log in</a> to see your friends\' activity on CNN.';
        }
    ...
    var CNNO = Class.create({
        ...
        'Page': {
            ...
            'getRefURL': function(add_params, sub_params) {
                ...
                if (Object.isUndefined(add_params)) {
                    add_params = {};
                }
                if (Object.isUndefined(sub_params)) {
                    sub_params = {};
                }
                ...
                //add new params
                for (var key in add_params) {
                    if (add_params[key]) {
                        t_arr.push(key + '=' + add_params[key]);
                    }
                }
...
CNNO.addMethods({
    ...
    'Overlay': {
        ...
        'callback': function(type, overlay_key) {
            ...
            if (type == 'json') {
                this.data[this.keys[overlay_key].j_file].j_loaded = 1;
            }
            if (type == 'html') {
                this.data[this.keys[overlay_key].h_file].h_loaded = 1;
            }
        ...
        'load': function(overlay_key, mode) {
            ...
            if (mode == 'auto') {
                ...
                //show loading html
                this.setDim(1);
                $('cnn_overlaycntnt').update('<div id="cnn_overlay_load" class="cnn_overlaybox cnn_overlayloading"><p align="center"><img src="../../../i.cdn.turner.com/cnn/.element/img/3.0/global/misc/loading.gif">loading</p></div>');
        ...
        'loadJSONP': function(overlay_key) {
            ...
            if (!this.data[this.keys[overlay_key].j_file].j_loaded) {
                ...
                this.callback('json', overlay_key);
            }
        ...
        'loadHCSI': function(overlay_key) {
            ...
            if (!this.data[this.keys[overlay_key].h_file].h_loaded) {
                ...
                this.callback('html', overlay_key);
            }
