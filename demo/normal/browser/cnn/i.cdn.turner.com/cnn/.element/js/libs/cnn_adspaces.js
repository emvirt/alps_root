...
function cnnad_parseReferrer(url) {
	...
	var data = url.substring(0, url.indexOf('http://weather.edition.cnn.com/', 7));
	if (data.indexOf(':', 5) > 0)
		data = data.substring(0, url.indexOf(':', 5));
	data = data.substring(data.lastIndexOf('http://weather.edition.cnn.com/')+1);
...
function cnnad_haveCookie(name) {
        return document.cookie && (-1 < document.cookie.indexOf('; '+name+'=') || document.cookie.indexOf(name+'=')==0);
}
...
// reload parent div tag of ad

function refreshAdsByDivId(divId, adClickCount) {
	var elems = document.getElementsByTagName("div");

 	for(var i = 0; i < elems.length; i++) {
 		var id = elems[i].id;
 		if(id.indexOf(divId) >= 0) {

 			if(elems[i].parentNode) {
 				var parent = elems[i].parentNode;
 				var html = parent.innerHTML; 
 				
 				// get tileid= and make it unique so ads can refresh properly
 				// this code will parse the div html, find tile= and increment count and
 				// update div with new tile				
 				var find = "&amp;tile=";
    			var tileStart = html.indexOf(find);
    			
    			if(tileStart > 0 ) {
	    			var tileEnd = html.indexOf("&amp;", tileStart+find.length);	    
	    			var tile = parseInt(html.substring(tileStart+find.length, tileEnd));	    			
	    			var newTile = tile+ adClickCount;	    			
	    			var find = "tile=" + tile;
	    			var replace = "tile=" + newTile;
